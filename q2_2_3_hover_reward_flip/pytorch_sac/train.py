...
from hover_wrapper import HoverRewardWrapper

def make_env(cfg):
    """Creates the environment with Hover Wrapper and a strict time limit."""
    env = gym.make(cfg.env, render_mode='rgb_array' if cfg.save_video else None)
    env = HoverRewardWrapper(env, hover_reward=200.0) 
...
class Workspace(object):
    ...
    def evaluate(self):
        """Offline evaluation requirement: 20 episodes every 10K steps."""
        print(f"\n>>> Triggering Evaluation at Step {self.step}")
        avg_reward = 0
        ...
        final_reward = avg_reward / self.cfg.num_eval_episodes
        self.logger.log('eval/episode_reward', final_reward, self.step)
        self.logger.dump(self.step, ty='eval')
        print(f">>> Evaluation Finished. Reward: {final_reward:.2f}\n")
    ...
    def run(self):
        ...
        while self.step < self.cfg.num_train_steps:
            if self.step % 100 == 0:
                print(f"Step: {self.step:6d} | Episode: {episode:4d} | Current R: {episode_reward:7.2f}", end="\r")
            ...
            if self.step == 300000:
                print("\n" + "!"*50 + "\nREWARD FLIP TRIGGERED: +200 -> -100\n" + "!"*50 + "\n")
                self.env.hover_reward = -100.0
            ...
            if done:
                ...
                obs, _ = self.env.reset(); done, episode_reward = False, 0; episode += 1; start_time = time.time()

            if self.step < self.cfg.num_seed_steps: action = self.env.action_space.sample()
            else:
